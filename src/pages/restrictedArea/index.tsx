...
export default function RestrictedArea() {
    ...
    return (
        <div className="flex h-screen w-full justify-center items-center bg-white">
            {instSelected && (
                InstituteModal({ instSelected, onClick: () => setInstSelected(null) })
            )}
            {isCreateActive && (
            ...
            <div className="flex flex-col w-[90%] items-center h-[90%] rounded-xl border-4 border-black bg-slate-300 shadow-sm">
                <div className="w-full border-b-4 flex items-center justify-between z-30 px-20 backdrop-blur-lg border-black h-[14%]">
                    ...
                    <div className="h-16 w-16 bg-light-purple rounded-lg hover:bg-violet hover:cursor-pointer" onClick={() => setIsCreateActive(true)} />
                </div>
