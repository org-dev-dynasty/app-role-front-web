...
export default function RestrictedArea() {
    ...
    return (
        <div className="flex h-screen w-full justify-center items-center bg-white">
            {instSelected && (
                InstituteModal({ instSelected, onClick: () => setInstSelected(null), setUpdate: () => setUpdateselected(true) })
            )}
            {updateselected && (
                InstituteModalUpdate({ instSelected, onClick: () => setUpdateselected(false), setUpdate: () => setUpdateselected(false) })
            )}
            {isCreateActive && (
            ...
            <div className="flex flex-col w-[90%] items-center h-[90%] rounded-xl border-4 border-black bg-slate-300 shadow-sm">
                <div className="w-full border-b-4 flex items-center justify-between z-30 px-10 backdrop-blur-lg border-black h-[14%]">
                    ...
                    <div className="h-16 w-16 bg-light-purple flex justify-center items-center rounded-lg hover:bg-violet hover:cursor-pointer" onClick={() => setIsCreateActive(true)} > <p className="text-3xl text-white">+</p> </div>
                </div>
                <div className="py-4 flex flex-col flex-1 overflow-y-auto w-full items-center gap-4">
                    {institutions.map((institution, index) => (
                        <InstitutionCard key={index} name={institution.name} onClick={() => setInstSelected(institution.name)} />
                    ))}
                </div>
            </div>
        </div>
